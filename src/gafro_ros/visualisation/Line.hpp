...
{
    class Line : public sackmesser_ros::Publisher<visualization_msgs::Marker, gafro::Line<double>>
    {
      public:
        Line(const std::string &ns, sackmesser_ros::Interface *interface);

        ~Line();

        visualization_msgs::Marker createMessage(const gafro::Line<double> &line) const;
