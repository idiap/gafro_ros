...
namespace gafro_ros::visualisation
{
    ...
    {
        return server->loadParameter("publisher/" + ns + "/frame", &frame) &&            //
    ...
    System::System(const std::string &name, sackmesser_ros::Interface *interface)
      : sackmesser_ros::Publisher<visualization_msgs::MarkerArray, gafro::System<double>, Eigen::MatrixXd, gafro::Motor<double>>(name, interface)
    {
    ...
    visualization_msgs::MarkerArray System::createMessage(const gafro::System<double> &system, const Eigen::MatrixXd &position,
                                                          const gafro::Motor<double> &base) const
    {
        ...
        {
            visualization_msgs::MarkerArray markers = convertToMarker(system, position.col(j), base, config_.frame);
